...
def generate_ellipsoid_clusters(distance, n_samples=100, cluster_std=0.5):
    ...
    shift_x = distance / np.sqrt(2)  # Split the shift equally along x and y directions
    shift_y = -distance / np.sqrt(2)
    X2 = np.random.multivariate_normal(mean=[1 + shift_x, 1 + shift_y], 
...
def logistic_loss(X, y, beta0, beta1, beta2):
    logits = beta0 + beta1 * X[:, 0] + beta2 * X[:, 1]
...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        loss = logistic_loss(X, y, beta0, beta1, beta2)
